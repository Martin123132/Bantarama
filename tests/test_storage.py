    ...
    def test_storage_uses_override_and_exports(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir="D:\\Temp" if Path("D:\\Temp").exists() else None) as tmp:
            ...
            try:
                ...
                state_after_house = storage.award_round("round-new", "The House")
                self.assertEqual(state_after_house["scores"]["The House"], 1)
                state_after_relic = storage.award_round("round-new", "The Relic")
                self.assertEqual(state_after_relic["scores"]["The Relic"], 1)
